...
export default function addContentHome() {
  ...
  function handleClickLocation(event) {
    ...
    renderLocation();
    if (locationButton == event.target) {
      locationButton.removeEventListener('click', handleClickLocation);
    }
  }
  function handleClickContactUs(event) {
    contentDiv.innerHTML = '';
    setupForm();
    if (callButton == event.target) {
      callButton.removeEventListener('click', handleClickLocation);
    }
  }
  function handleClickOrderNow(event) {
    contentDiv.innerHTML = '';
    renderMenu();
    if (orderButton == event.target) {
      orderButton.removeEventListener('click', handleClickLocation);
    }
  }
  function handleClickAboutUs(event) {
    contentDiv.innerHTML = '';
    renderAboutUs();
    if (learnBtn == event.target) {
      learnBtn.removeEventListener('click', handleClickLocation);
    }
  }
  //dom manipulation
  ...
  locationButton.addEventListener('click', handleClickLocation);
  learnBtn.innerHTML = 'Learn More About Us';
  learnBtn.addEventListener('click', handleClickAboutUs);
  ...
  orderButton.addEventListener('click', handleClickOrderNow);
  callButton.classList.add('button-def', 'button-5', 'call-btn');
  callButton.textContent = 'Contact Us';
  callButton.addEventListener('click', handleClickContactUs);
  foodUno.src = foodOne;
